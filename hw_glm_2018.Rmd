...
# Tobit model for mixed discrete or continuous data: 
...
- treat: 1 = experimental treatment group (NSW); 0 = comparison group (either from CPS or PSID)   - Treatment took place in 1976 and 1977.
- age  = age in years
...
```{r}
summary(fit.8 <- vglm(data = congress,`1988`~.,family =gaussianff ))
#anova(fit.8,fit.7)
ggplot()+aes(x=fitted(fit.8),y=residuals(fit.8)[,1])+geom_point()+geom_smooth(method = "loess")
rSquared(resid = residuals(fit.8)[,1],y = drop_na(select(congress,`1988`))%>%pull())
```
...
```{r}
congressbi = congress%>%mutate(`1988` = if_else(`1988`>0.5,1.0,0.0))
summary(fit.9 <-  glm(data = congressbi , `1988`~ . ,family = binomial(link = "logit")))
binnedplot(x = fit.9$fitted,y = fit.9$resid)
```
...
```{r}
ggplot(salmonella)+aes(x = log(dose),y = log(colonies))+geom_point()
```

This shows that the trend is not monotonic.  Hence when you fit the model and look at the residual you will see a trend.
```{r}
fit.10 <- glm(data = salmonella,colonies ~ dose,family = poisson(link = "log"))
summary(fit.10)
ggplot(fit.10)+aes(x = .fitted,y =.stdresid)+geom_point()
```
