...
ggplot(lmer.coef.1.add)+
  ...
  geom_abline(aes(intercept = intercept.3[3],slope = lmer.coef.2.pool[3],color = "Complete pooling"),size = 1.5)+
  geom_abline(aes(intercept = lm.coef$coef.fit.lm...1.[3],slope = fit.lm.1,color = "No pooling"),linetype ="dashed",size = 1.5)+
  ylim(c(0,7.5))+
  xlim(c(0,2))+
  theme(legend.position="bottom")+
...
```{r}
application <-  1:100
round.1 <- sample(application,100,replace = FALSE)
round.2 <- sample(application,100,replace = FALSE)
round.3 <- sample(application,100,replace = FALSE)
```
